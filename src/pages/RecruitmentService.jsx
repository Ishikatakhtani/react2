import BacktoTop from "./BacktoTop";
import  { useEffect } from 'react';
const RecruitmentService=()=>{
   useEffect(() => {
    document.title = "Recruitment Service |  Yashika Counsulting Services";
  }, []);
    return(
        <>
        <div id="Home4">
      <section class="recruitment-section">
    <div class="content-wrapper">
      <h1>Our Professional Services for Your <br /> Recruitment Needs</h1>
      <p>
        Looking for your dream job or exceptional talent for your organization? Look no further than our top-notch recruitment agency. We are committed to aiding both job seekers and employers in discovering their ideal matches, ensuring a seamless, efficient, and rewarding process. Our dedication to excellence sets us apart, making us the preferred choice for all your recruitment needs.
      </p>
    </div>
  </section>

<section class="mentorship">
  <div class="mentor-card">
    <img src="https://static.wixstatic.com/media/nsplsh_7576355f6273797046554d~mv2_d_5472_3648_s_4_2.jpg/v1/crop/x_923,y_0,w_3648,h_3648/fill/w_348,h_348,al_c,q_80,usm_0.66_1.00_0.01,enc_avif,quality_auto/Image%20by%20Hack%20Capital.jpg" alt="Software Professional"/>
    <h3>Personalized Approach</h3>
    <p>
Opting for our services brings forth a multitude of benefits. As a premier recruitment agency, we are dedicated to assisting both job seekers and employers in finding their perfect matches. Whether you are in search of your dream job or exceptional talent for your organization, we guarantee a smooth, efficient, and gratifying process. Our unwavering commitment to excellence distinguishes us, making us the preferred option for all your recruitment requirements.
  </p>
  </div>

  <div class="mentor-card">
    <img src="https://static.wixstatic.com/media/11062b_ae708cec5f3242a588b44741a32efc05~mv2.jpg/v1/crop/x_968,y_0,w_2160,h_2160/fill/w_348,h_348,al_c,q_80,usm_0.66_1.00_0.01,enc_avif,quality_auto/Data%20Technology.jpg" alt="Senior Technologist"/>
    <h3>Seamless Process</h3>
    <p>
Looking for a job or finding the perfect candidate can be a time-consuming and daunting task. However, we simplify the entire process, making it hassle-free and efficient for you. From conducting initial screenings and shortlisting candidates to coordinating interviews and negotiating offers, we take care of all the complexities involved. This allows you to concentrate on your core priorities without getting bogged down by the recruitment process.

 </p>
  </div>

  <div class="mentor-card">
   <img src="https://static.wixstatic.com/media/11062b_7674c3caf4e44ef59438591a09bb5170~mv2.jpg/v1/fill/w_348,h_348,al_c,q_80,usm_0.66_1.00_0.01,enc_avif,quality_auto/Chess.jpg" alt="" />
    <h3>Expertise</h3>
    <p>
    Benefiting from extensive experience in the recruitment industry, we have acquired profound expertise in market trends, emerging technologies, and the ever-changing dynamics of the job market. By staying at the forefront of developments, we offer you invaluable insights and guidance throughout your recruitment journey.
       </p>
  </div>
</section>

<BacktoTop/>
     </div>   </>
    )
}
export default RecruitmentService;